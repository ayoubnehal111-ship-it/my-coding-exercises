...
During the module detection step (before merging), the object modulecolors_W contains all detected modules, including the grey module corresponding to unassigned genes. In this context, subtracting one from the number of unique module colors correctly removes the grey module, resulting in 40 co-expression modules, which is consistent with the value indicated in the script comments.
According to the comments in the R script, the expected number of co-expression modules is 40 before merging and 36 after merging. After running the script, the result obtained for the module detection step was consistent with this expectation (40 modules before merging). However, after the module merging step, the numerical output displayed 35 modules instead of 36, which prompted further investigation.
During the module merging step, the object modulecolors_W is replaced by the merged module assignments generated by the mergeCloseModules() function. At this stage, the treatment of the grey module differs: it may be absent, empty, or implicitly excluded from the merged module color vector. Consequently, applying the same counting formula (length(unique(modulecolors_W)) - 1) may remove one additional module, leading to an apparent count of 35 instead of 36, alternatively, there may be a discrepancy in the number indicated in the script comments.
So based on the R script we have the result shown by the line command “print(length(unique(modulecolors_W)) - 1)” was 35 after murging and 40 before.
